...
class Qwen3OmniMoeThinkerForConditionalGeneration(
    ...
    def get_mrope_input_positions(
        ...
        if audio_feature_lengths is not None and not isinstance(
            ...
            audio_feature_lengths = torch.as_tensor(audio_feature_lengths,
                                                    dtype=torch.long)
        if second_per_grid_ts is None:
            ...
            second_per_grids = torch.tensor(second_per_grid_ts,
                                            dtype=torch.float32)
            ...
            if min_ed == ed_audio_start:
                ...
                if text_len != 0:
                    ...
                    llm_pos_ids_list.append(
                        torch.arange(text_len, dtype=torch.long).view(
                            1, -1).expand(3, -1) + st_idx)
                st_idx = llm_pos_ids_list[-1].max(
                ) + 1 if llm_pos_ids_list else 0
                bos_len = 1
                llm_pos_ids_list.append(
                    torch.arange(bos_len, dtype=torch.long).view(1, -1).expand(
                        3, -1) + st_idx)
                st_idx = llm_pos_ids_list[-1].max(
                ) + 1 if llm_pos_ids_list else 0
                _, audio_len = _get_feat_extract_output_lengths(
                    audio_feature_lengths[audio_idx])
                llm_pos_ids = (torch.arange(audio_len, dtype=torch.long).view(
                    1, -1).expand(3, -1) + st_idx)
                ...
                eos_len = 1
                llm_pos_ids_list.append(
                    torch.arange(eos_len, dtype=torch.long).view(1, -1).expand(
                        3, -1) + st_idx)
                ...
                bos_len = 1
                bos_block = (torch.arange(bos_len, dtype=torch.long).view(
                    1, -1).expand(3, -1) + st_idx)
                ...
                _, audio_len = _get_feat_extract_output_lengths(
                    audio_feature_lengths[audio_idx])
                audio_llm_pos_ids = (torch.arange(
                    audio_len, dtype=torch.long).view(1, -1).expand(3, -1) +
                                     st_idx)
                grid_t = video_grid_thw[video_idx][0]
                grid_hs = video_grid_thw[:, 1]
                grid_ws = video_grid_thw[:, 2]
                ...
                eos_len = 1
                eos_block = (torch.arange(eos_len, dtype=torch.long).view(
                    1, -1).expand(3, -1) + st_idx)
        ...
        if st < len(input_tokens):
            ...
            llm_pos_ids_list.append(
                torch.arange(text_len, dtype=torch.long).view(1, -1).expand(
                    3, -1) + st_idx)
