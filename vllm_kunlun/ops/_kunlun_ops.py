...
class KunlunOps:
    ...
    def paged_attention_v1(output,
        ...
        xtorch_ops.paged_attention(x=query,
                                   k_cache=key_cache,
                                   v_cache=value_cache,
    ...
    def rotary_embedding(positions, query, key, head_size, cos_sin_cache,
        ...
        torch.ops._C.rotary_embedding(positions, query_x, key_x, head_size,
    ...
    # Rotary embedding
    @staticmethod
    def mrotary_embedding(positions, mrope_section, query, key, head_size,
                          cos_sin_cache, is_neox_style):
        """
        refactor RotaryEmbedding forward function
        """
        ...
        xtorch_ops.mrotary_embedding_neox(positions, query_x, key_x, head_size,
                                          cos_sin_cache, mrope_section)
    ...
    @staticmethod
    def multi_query_kv_attention(usual_seq_lod_xpu: torch.Tensor,
                                 usual_seq_lod_cpu: torch.Tensor,
                                 query: torch.Tensor, key: torch.Tensor,
                                 value: torch.Tensor, **kargs) -> torch.Tensor:
        """
    ...
    def quant_fusedresidual_rmsnorm_op(x,
        ...
        if is_per_token_smooth_quant():
            out_scale = torch.empty(x.shape[:-1],
                                    device=x.device,
                                    dtype=torch.float).unsqueeze(-1)
        else:
            out_scale = torch.empty(12, device=x.device, dtype=torch.float)

        xtorch_ops.quant_fusedresidual_rmsnorm(x,
                                               residual,
                                               weight,
                                               bias,
                                               eps,
    ...
    @staticmethod
    def smooth_quant_matmul_column_row_kernels(input_tensor, weight, smoother,
                                               input_scale, weight_scale,
                                               perTokenScaling,
                                               perChannelScaling, otype):
    ...
    def fused_moe(
        ...
        if scoring_func == "softmax":
            torch.ops._C.moe_softmax_topk_norm(x=router_logits,
        ...
        if w1_bias is not None or w2_bias is not None:
            ...
            for i in range(global_num_experts):
                ...
                if selected_token.sum():
                    ...
                    up_gate = torch.empty(selected_token.sum(),
                                          up_gate_size // 2,
            ...
            ouput = (out.view(M, moe_top_k, N) *
                     normed_score.unsqueeze(2)).sum(dim=1).to(
                         hidden_states.dtype)
            return ouput
        else:
            ...
            sorted_tokens_idx = torch.zeros(M * moe_top_k,
                                            dtype=torch.int32,
                                            device=hidden_states.device)
            ...
            dequant_scale = torch.ones([M, moe_top_k],
                                       dtype=torch.float32,
                                       device=out.device)
    ...
    def fused_moe_ep(
        ...
        topk_ids = torch.empty(batch,
                               ...
                               dtype=torch.int32,
                               device=router_logits.device)
        block_static = torch.empty(0,
                                   dtype=torch.int32,
                                   device=router_logits.device)
        torch.ops._C.moe_softmax_topk(router_logits, topk_weights, topk_ids,
                                      block_static)
        ...
        output = (out.view(batch, top_k, hidden_size) *
                  topk_weights.unsqueeze(2)).sum(dim=1).to(x.dtype)
    ...
    def fused_recurrent_gated_delta_rule_fwd(
        ...
        o, final_state = xtorch_ops.fused_recurrent_gated_delta_rule_fwd(
            q, k, v, g, beta, scale, h0_source, output_final_state,
