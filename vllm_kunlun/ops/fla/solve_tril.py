    ...
    for loopX in range(NT):
        ...
        for loopY in range(B * H):
            ...
            if (bos * H + i_h) < H:
                ...
                BTstart = loopX * 16 % BT
                BTend = BTstart + 16
                subA = A[0, Tstart:Tend, loopY,
                         BTstart:BTend].contiguous().clone()
                ...
                if subA.shape[0] < 16:
                    ...
                    zeros = torch.zeros((pad_rows, subA.shape[1]),
                                        dtype=subA.dtype,
                                        device=subA.device)
            ...
            if (bos * H + i_h) < H:
                ...
                BTstart = 0 * 16
                BTend = BTstart + 16
                subAd = Ad_modify[0, Tstart:Tend, loopY,
                                  BTstart:BTend].contiguous().clone()
                # print(f'T={T}, Tstart={Tstart}, Tend={Tend}, BTstart={BTstart}, BTend={BTend}')
            else:
                assert (0) & "need deal this situation"

            mask = (torch.arange(16, device=subA.device)[:, None]
            ...
            for inLoopIdx in range(1, min(16, T - i_t * 16)):
                ...
                AInLoop = A[0, (loopX * 16 + inLoopIdx), loopY,
            ...
            subAd = subA + (torch.arange(16, device=subA.device)[:, None]
                            == torch.arange(16, device=subA.device)[None, :])
            ...
            if (Tend > T):  # bondary mask
                needMaskRow = Tend - T
                Ad_modify[0, Tstart:Tend, loopY,
                          BTstart:BTend] = subAd[:T - Tstart, :]
            else:
